...
class VehicleReception(db.Model):
    ...
    vehicle_id = db.Column(db.Integer, db.ForeignKey('vehicles.id', onupdate='RESTRICT', ondelete='CASCADE'))
    vehicle = db.relationship('Vehicle', backref=db.backref('vehicle_receptions', lazy=True))
    employee_id = db.Column(db.Integer, db.ForeignKey('employees.id', onupdate='RESTRICT', ondelete='CASCADE'))
    employee = db.relationship('Employee', backref=db.backref('vehicle_receptions', lazy=True))
