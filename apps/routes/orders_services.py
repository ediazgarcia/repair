...
orders_services = Blueprint('orders_services', __name__, url_prefix='/orders_services')
...
def create_orders_services(user=None):
    try:
        if request.method=='POST':
            ...
            vehicle_reception = VehicleReception.query.filter_by(id=vehicle_reception_id).first()
            employee = Employee.query.filter_by(id=employee_id).first()
            product = Product.query.filter_by(id=product_id).first()

            new_order=ServiceOrder(description=description, observations=observations, start_date=start_date, end_date=end_date, status=status, vehicle_reception=vehicle_reception, employee=employee, product=product)
